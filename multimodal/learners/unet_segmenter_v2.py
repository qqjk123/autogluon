    ...
    def fit_from_nnunet(
        ...
        def make_loader(ex_list, shuffle):
            ...
            transforms = [
                ...
                Lambdad(
                    keys="label",
                    func=RemapLabels({int(k): i for i, k in enumerate(labels)}),
                ),
                ScaleIntensityRanged(
                ...
                RandCropByPosNegLabeld(
                    keys=["images", "label"],
                    label_key="label",
                    spatial_size=(96, 96, 96),  # 更大训练 crop
                    pos=2,
                    neg=1,
                    num_samples=4,
                ),
        ...
        for ep in range(1, epochs + 1):
            ...
            with torch.no_grad():
                for batch in val_loader:
                    ...
                    self.val_metric(
                        y_pred=one_hot(
                            pred_lbl.unsqueeze(1), self.num_classes
                        ),
                        y=one_hot(
                            batch["label"].unsqueeze(1).to(self.device),
                            self.num_classes,
                        ),
                    )
            ...
            print(
                ...
                f"(best {best_dice:.4f}, no_improve {epochs_no_improve})"
            )

            if epochs_no_improve >= self.early_stop_patience:
                print(f"Early stopping at epoch {ep}, best val_dice: {best_dice:.4f}")
                break
    ...
    def evaluate_from_nnunet(
        ...
        transforms = [
            ...
            Lambdad(
                keys="label",
                func=RemapLabels(
                    {int(k): i for i, k in enumerate(sorted(info["labels"].keys(), key=int))}
                ),
            ),
            ScaleIntensityRanged(
                keys=["images"], a_min=0, a_max=3000, b_min=0.0, b_max=1.0, clip=True
        ...
        loader = DataLoader(
            CacheDataset(data=data, transform=transforms, cache_rate=0.0),
            batch_size=batch_size,
            shuffle=False,
            num_workers=num_workers,
            pin_memory=torch.cuda.is_available(),
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                ypb = (pred_lbl > 0)
                ytb = (batch["label"].to(self.device) > 0)
                tp += int((ypb & ytb).sum())
                tn += int((~ypb & ~ytb).sum())
                fp += int((ypb & ~ytb).sum())
                fn += int((~ypb & ytb).sum())
        ...
        print(
            f"Test Results → Dice: {mean_dice:.4f} ± {dice_var:.6f} "
            f"(95% CI [{dice_ci_l:.3f}, {dice_ci_u:.3f}])"
        )
        print(
            f"             HD95: {mean_hd95:.2f} ± {hd95_var:.4f} "
            f"(95% CI [{hd95_ci_l:.2f}, {hd95_ci_u:.2f}])"
        )
        print(
            f"       Sensitivity: {sensitivity:.4f} ± {sens_var:.6f} "
            f"(95% CI [{sens_ci_l:.3f}, {sens_ci_u:.3f}])"
        )
        print(
            f"       Specificity: {specificity:.4f} ± {spec_var:.6f} "
